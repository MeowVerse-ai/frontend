...
import { Mail, Lock, Sparkles, Key, X } from 'lucide-react';
...
const LoginPage = () => {
  ...
  const handleGoogleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setLoading(true);
      try {
        const result = await authService.googleOAuth(tokenResponse.access_token);
        ...
        // Check if error is about beta code requirement (new user)
        if (errorMessage.includes('Beta access code is required')) {
  ...
  const handleBetaCodeSubmit = async (e) => {
    e.preventDefault();
    if (!betaCode.trim()) {
    ...
    setLoading(true);
    try {
      const result = await authService.googleOAuth(googleAccessToken, betaCode);
      localStorage.setItem('accessToken', result.data.accessToken);
      localStorage.setItem('refreshToken', result.data.refreshToken);
      setUser(result.data.user);
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="relative max-w-md w-full">
            ...
            <div className="relative bg-slate-900 rounded-3xl p-8 border border-purple-500/30">
              ...
              <h2 className="text-2xl font-bold text-white mb-2">Beta Access Required</h2>
              <p className="text-gray-400 mb-6">
                Enter your beta access code to complete registration with Google.
              </p>
